// key-actions
export const ADD_KEYDOWN = 'keys/KeyDown';
export const ADD_KEYUP = 'keys/KeyUp';
// game-actions
export const PAUSE_GAME = 'game/pauseGame';
export const CONTINUE_GAME = 'game/continueGame';
export const ADD_DIFFICULTY = 'game/difficulty';
// music-actions
export const REPLACE_ARTIST_NAME = 'music/changeArtist';
export const REPLACE_TITLE_NAME = 'music/changeTitle';
export const STOP_MUSIC = 'music/stopMusic';
export const PLAY_MUSIC = 'music/playMusic';
// canvas-actions
export const SCALE_CANVAS = 'canvas/Scale';